...
$N=3,4,5,\dotsc$ とする. 任意の$N$角形は　$m$ 個の凸多角形に分解される. ここに $m=1,\dotsc,N-2$ である. 凸多角形とは多角形であり多角形内の任意の2点を結ぶ線分が多角形に含まれることである. 凸$N$角形は $N-2$ 個の三角形に分割されるので内角の和は $(N-2)\times180^\circ$ であり, したがって任意の$N$角形の内角の和は $(N-2)\times180^\circ$ である. 
